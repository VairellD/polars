    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            <div class="flex-shrink-0 flex items-center">
                <a href="/" class="block">
                    <span class="text-xl font-semibold">
                        <img src="{{ asset('assets/Terminal_LOGO.svg') }}" alt="PoLaRs." class="h-6 w-auto">
                    </span>
                </a>
            </div>
                    ...
                    @auth
                        @if(Auth::user()->is_admin)
                            <a href="{{ route('admin.profile') }}"
                                class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->is('admin/profile') ? 'border-purple-600 text-gray-900 font-semibold' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                                ADMIN PROFILE
                            </a>
                        @else
            ...
            <div class="hidden sm:flex sm:items-center sm:space-x-4">
                <form action="{{ route('posts.search') }}" method="GET" class="relative flex items-center">
            ...
            @auth
                @if(Auth::user()->is_admin)
                    <a href="{{ route('admin.profile', Auth::user()) }}"
                        class="block pl-3 pr-4 py-2 border-l-4 {{ request()->is('admin/profile') ? 'border-purple-600 text-purple-700 bg-purple-50' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800' }}">
                        ADMIN PROFILE
                    </a>
                @else
